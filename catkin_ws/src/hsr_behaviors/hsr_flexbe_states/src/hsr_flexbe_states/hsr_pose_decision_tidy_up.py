...
class hsr_PoseDecisionTidyUp(EventState):
    '''
    ...
    def __init__(self, offset=None, tolerance=0.001, multi_plan=False, multi_plan_num=3, orien_tolerance=True):
        super(hsr_PoseDecisionTidyUp, self).__init__(outcomes=['continue', 'plan_failed', 'detect_failed'], input_keys=['target_poses', 'target_object_names'], output_keys=['selected_pose_approach', 'selected_pose_grasp', 'selected_object_name', 'selected_object_status'])
        self._offset = offset
