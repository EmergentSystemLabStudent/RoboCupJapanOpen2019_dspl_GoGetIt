...
import rospy, rosparam, cv2, cv_bridge, os, subprocess, tf2_ros, tf2_geometry_msgs, chainer, time#, sys
import numpy as np
...
class hsr_SpaCoTyGetDatasetPlus(EventState):
                ...
                for n, region in enumerate(result.regions):
                    ...
                    if pixel_count != 0:
                        ...
                        try:
                            ...
                            self.object_data = np.append(self.object_data, resnet_vec['pool5'].data[0])
    ...
    def on_enter(self, userdata):
        ...
        subprocess.Popen("cp " + self._save_folder[:-len(userdata.save_folder)] + "word_list_st" + userdata.save_folder[-1:] +".csv " + self._save_folder, shell=True)
        subprocess.Popen("mv " + self._save_folder + "/word_list_st" + userdata.save_folder[-1:] +".csv " + self._save_folder + "/word_list.csv", shell=True)
        time.sleep(0.1)
