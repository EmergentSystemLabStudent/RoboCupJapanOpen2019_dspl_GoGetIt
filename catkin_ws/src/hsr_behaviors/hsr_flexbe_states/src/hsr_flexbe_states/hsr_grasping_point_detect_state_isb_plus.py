...
import sys, rospy, cv2, cv_bridge, os, chainer
import numpy as np
...
class hsr_GraspingPointDetectIsbPlus(EventState):
    '''
    ...
    #> grasping_point_feature    dict           grasping_point_feature
    ...
    def __init__(self, output_topic="/bounding_box_2d_monitor", resnet_model="resnet50", save=True):
        super(hsr_GraspingPointDetectIsbPlus, self).__init__(outcomes=['completed', 'failed', 'yolo_failed'], input_keys=['rgb_image', 'depth_image', 'camera_info'],
                                                output_keys=['grasping_point', 'grasping_point_feature'])
        ...
        if resnet_model[-2:] == '50':
            ...
            print("[ERROR] You must choice \'ResNet-50\', \'ResNet-101\' or \'ResNet-152\'.")
            exit(1)
    ...
    def execute(self, userdata):
        ...
        for n, region in enumerate(result.regions):
            ...
            dst_im = rgb_image_init[int(y0):int(y0 + region.height), int(x0):int(x0 + region.width)]
            ...
            if pixel_count != 0:
                ...
                # save the object features
                dst_im = cv2.resize(dst_im, (224, 224), interpolation = cv2.INTER_CUBIC)
                dst_im_conv = chainer.links.model.vision.resnet.prepare(dst_im)
                dst_im_conv = dst_im_conv[np.newaxis]
                resnet_vec = self.resnet.extract(dst_im_conv)
                dict_feature.update({result.names[n]: resnet_vec['pool5'].data[0]})

                #dict_label.setdefault(result.names[n],[]).append(pose)
                dict_label.update({result.names[n]: pose})
        ...
        userdata.grasping_point = dict_label
        userdata.grasping_point_feature = dict_feature
