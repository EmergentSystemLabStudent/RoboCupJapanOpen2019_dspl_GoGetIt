...
class hsr_GraspingPointDetect(EventState):
    '''
    ...
    def __init__(self, output_topic="/bounding_box_2d_monitor", save=True):
        super(hsr_GraspingPointDetect, self).__init__(outcomes=['completed', 'failed'], input_keys=['rgb_image', 'depth_image', 'camera_info', 'detection'],
    ...
    def execute(self, userdata):
        ...
        if not result is None:
            Logger.loginfo('Finished detection.')
            return 'completed'
        else:
            return 'failed'
